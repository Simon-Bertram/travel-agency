...
One Turborepo product, not a separate admin app. Public marketing and deals live on Astro. Staff UI lives under `/admin` and is strictly protected by Better Auth middleware.
...
- **Hono + Workers** (`apps/server/src/index.ts`): business logic, Better Auth at `/api/auth/*`, oRPC at `/rpc`. Infra is Alchemy in `packages/infra`, not wrangler.toml.
